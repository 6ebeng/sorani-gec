#!/bin/bash
# =============================================================================
# Phase 1 retrain — convergence + deconfounded morphaware (audit 2026-05-30)
# =============================================================================
# One-command relaunch of the two headline models with the Phase 1 fixes from
# notes/thesis_critical_audit_2026-05-30.md. It is GPU-bound: run it on the
# rental RTX 5090 (vast.ai), not on a CPU box.
#
# What changes versus run_phase_d_seeds.sh (the released runs):
...
#                            result (L9-02/L8-02).
#
# Seeds: 42 123 777 (the released set; see L9-03). Eval uses scripts/eval_phase_d.py
...
#
# Usage:
#   bash scripts/phase1_retrain.sh                 # 3 seeds, both models
#   bash scripts/phase1_retrain.sh --seeds "42"    # single-seed smoke test
#   bash scripts/phase1_retrain.sh --baseline-only
#   bash scripts/phase1_retrain.sh --morph-only
#
# Exit gate:
#   Six checkpoints under results/phase1/, each selected on val-F0.5 plateau;
...
echo "==================================================================="
echo "Phase 1 retrain (convergence + deconfounded morphaware)"
echo "  Seeds:        $SEEDS"
echo "  Data:         $DATA_DIR"
...
echo ""
echo "==================================================================="
echo "Phase 1 retrain complete — $(date -u '+%Y-%m-%d %H:%M:%S UTC')"
echo "Next: evaluate each checkpoint with the unified scorer, then rerun bootstrap."
echo ""
echo "  # both eval scripts honour PHASE_RESULTS_DIR / PHASE_DATA_DIR"
echo "  export PHASE_RESULTS_DIR=$RESULTS_DIR"
echo "  export PHASE_DATA_DIR=$DATA_DIR"
echo "  python scripts/eval_phase_d.py        # aggregate F0.5 mean+/-std per model"
echo "  python scripts/dump_hypotheses.py     # write per-seed hypotheses.jsonl"
echo ""
echo "  # point run_bootstrap.py at the new hypotheses (edit PHASE_D in the script"
echo "  # or copy results/phase1/*/hypotheses.jsonl into results/phase_d/), then:"
