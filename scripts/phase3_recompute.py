"""Phase 3 recompute — span-aware headline metrics from released hypotheses.
...
be dropped from the denominator. Both phase1 (deconfounded retrain) and phase_d
(original campaign) store source/hypothesis/reference per test sentence, so the
span-aware numbers come straight from those files — no GPU.
...
Output: results/phase3_metrics.json
"""
...
def main():
    ...
    out_path = ROOT / "results" / "phase3_metrics.json"
