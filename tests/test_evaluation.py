...
def test_agreement_checker_object_verb_ergative():
    ...
    assert result.checks_total == 16, (
        f"Expected 16 checks (including ergative + new checks), got {result.checks_total}"
    )
...
def test_agreement_checker_fourteen_checks_counted():
    """AgreementChecker must run exactly 16 checks per sentence."""
    checker = AgreementChecker()
