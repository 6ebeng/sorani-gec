...
Δ F₀.₅ = +0.0048 in favour of morphology; paired bootstrap (10,000 resamples) p = 0.39 — the margin does not clear significance. The honest headline: after all data bugs are fixed, morphology injection no longer _hurts_ (it did in campaign 2), and trends slightly ahead on precision.
...
Run on the early (pre-cleanup) data, so absolute values are tiny; the _relative_ ordering informed the final configuration:
