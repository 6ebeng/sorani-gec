...
**My F₀.₅ does not match the published number.** Check three things: word-level vs span-level scorer (the thesis reports span), `max_length` at generation time (512 for campaign 3; 256 clips long test sentences), and which campaign/checkpoint you loaded (HF hosts campaign 2 — multiseed — not the clean-final models; snapshots created before the directory rename may have a different parent path — see [[Reproducibility]]).
